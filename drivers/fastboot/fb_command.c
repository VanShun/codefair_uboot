...
static void flash(char *cmd_parameter, char *response)
{
	...
	char spi_parameter[64];
	int len;

	/* Delete cmd parameter "_a" */
	strcpy(spi_parameter, cmd_parameter);
	len = strlen(cmd_parameter);
	spi_parameter[len - 2] = '\0';
	fastboot_spi_flash_write(spi_parameter, fastboot_buf_addr, image_size,
				  response);
#endif
}
...
static void erase(char *cmd_parameter, char *response)
{
	...
	len = strlen(cmd_parameter);
	spi_parameter[len - 2] = '\0';
	fastboot_spi_erase(spi_parameter, response);
#endif
}
#endif
