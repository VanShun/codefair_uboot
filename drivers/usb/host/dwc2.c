				     ...
				     struct devrequest *cmd)
{
	...
	uint16_t wValue = cpu_to_le16(cmd->value);
	uint16_t wIndex = cpu_to_le16(cmd->index);
    uint32_t regl1;
	...
	case (USB_REQ_SET_FEATURE << 8) | USB_RECIP_OTHER | USB_TYPE_CLASS:
		...
		case USB_PORT_FEAT_ENABLE:
				puts("enter USB_PORT_FEAT_ENABLE\n");
			break;
		case USB_PORT_FEAT_TEST:
			puts("enter USB_PORT_FEAT_TEST\n");
			//clrsetbits_le32(&regs->hprt0, DWC2_HPRT0_PRTTSTCTL_MASK);
			regl1 = readl(&regs->hprt0);
			printf("regl1====%x\n",regl1);
			regl1 &= ~DWC2_HPRT0_PRTTSTCTL_MASK ;
			regl1 |= (wIndex >> 8) << DWC2_HPRT0_PRTTSTCTL_OFFSET;
			writel(regl1,&regs->hprt0);
			regl1 = readl(&regs->hprt0);
			printf("regl2====%x\n",regl1);
			break;
		}
